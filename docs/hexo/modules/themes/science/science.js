...
const HEXAGON_SIDE = 33;
const HEXAGON_W = Math.round(HEXAGON_SIDE * Math.sqrt(3));
const HEXAGON_H = HEXAGON_SIDE * 3 / 2;
...
export function draw_cell(i, j) {
    ...
    const x_min = x - HEXAGON_W / 2;
    const x_max = x + HEXAGON_W / 2;
    
    const y_up = y - HEXAGON_SIDE / 2;
    const y_down = y + HEXAGON_SIDE / 2;
    
    const cell = document.createElementNS('http://www.w3.org/2000/svg', 'polygon');
    cell.setAttributeNS(null, 'points',
        `${x},${y - HEXAGON_SIDE} ${x_max},${y_up} ${x_max},${y_down} ${x},${y + HEXAGON_SIDE} ${x_min},${y_down} ${x_min},${y_up}`
    );
